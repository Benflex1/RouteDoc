...
| `ListenerInventoryResult` | `namespace_entity_id`, `protocol`, `address_family`, `bind_semantics`, `port_start`, `port_end`, `matching_listener_count` |
...
**Failing tests first:** a direct exact-scope zero-count inventory result and a broader contained zero-count result fire; complete visibility without a result, the former non-target-entry workaround, partial/unknown/not-applicable visibility, denied/unavailable collection, namespace/vantage/protocol/family/bind/port/process-ownership scope mismatch, and wrong-vantage TCP result do not; broader nonzero aggregates do not prove target absence; an exact-target nonzero result and a positive matching listener contradict/suppress absence; process-ownership accounting uses distinct concrete listener identities. Assert exact rule ID, mandatory same-rule claims, direct result and visibility references, deterministic key, and evaluator/persisted-validator equivalence. Include complete and partial fixture-shaped cases. Assert `DefaultRegistry()` contains exactly the three exemplar IDs once each in ascending order.

**Observe failure:** `GOTOOLCHAIN=go1.26.5 go test ./internal/rules/listener`.

**Implement:** `NewNoMatchingListenerVisible() ruleapi.Rule`; require a matching `COMPLETE_FOR_SCOPE` grounded by a qualifying `LISTENER_INVENTORY_RESULT` in the exact vantage/namespace/protocol/family/bind scope, with inclusive result/visibility containment and a zero count for the existing single target port (treated as `[P,P]` only for containment). Reject coherent positive entries and exact-target nonzero results, while not locating a target listener from a broader nonzero aggregate. The support path includes the visibility assessment and every basis observation, and it never infers process state, firewall state, or historical cause. Assemble the immutable parent-package `DefaultRegistry` now that all subpackages exist; no init-time mutable registration is allowed.

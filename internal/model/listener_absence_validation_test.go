...
func TestPersistedListenerAbsenceClaimUsesVisibilityContract(t *testing.T) {
	cases := []struct {
		...
		{name: "positive matching listener", mutate: func(r *EvaluatedRun) {
			r.Evidence.Observations = append(r.Evidence.Observations, Observation{ObservationID: "observation-000002", Kind: ObservationListenerInventory, SubjectEntityIDs: []EntityID{"entity-listener", "entity-namespace"}, VantageID: r.Evidence.Observations[0].VantageID, ObservedAt: r.Evidence.StartedAt, Payload: ObservationPayload{Kind: ObservationListenerInventory, Listener: &ListenerInventoryEntry{ListenerEntityID: "entity-listener", NamespaceEntityID: "entity-namespace", Protocol: TransportTCP, AddressFamily: AddressFamilyIPv4, BindSemantics: BindWildcard, Port: 443}}, AcquisitionMethod: AcquisitionSyntheticFixture, SourceComponent: SourceSyntheticFixture, Sensitivity: SensitivitySanitizedDerived, Limitations: []Limitation{}})
		}},
		{name: "wrong port scope", mutate: func(r *EvaluatedRun) {
			r.Evidence.Observations[0].Payload.ListenerInventoryResult.PortEnd = 100
		}},
		{name: "zero basis", mutate: func(r *EvaluatedRun) {
			r.Evidence.VisibilityAssessments[0].BasisObservationIDs = nil
		}},
		{name: "broader nonzero result", mutate: func(r *EvaluatedRun) {
			r.Evidence.Observations[0].Payload.ListenerInventoryResult.MatchingListenerCount = 1
		}},
		{name: "exact target nonzero result without entry", mutate: func(r *EvaluatedRun) {
			r.Evidence.Observations[0].Payload.ListenerInventoryResult.PortStart = 443
			r.Evidence.Observations[0].Payload.ListenerInventoryResult.PortEnd = 443
			r.Evidence.Observations[0].Payload.ListenerInventoryResult.MatchingListenerCount = 1
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.valid && !hasValidationCode(issues, CodeVisibilityInsufficientForAbsence) && !hasValidationCode(issues, CodeVisibilityScopeMismatch) {
...
func TestPersistedListenerAbsenceRejectsNonTargetEntryCompletionWorkaround(t *testing.T) {
	r := listenerAbsenceEvaluatedRun()
	r.Evidence.Observations[0].Kind = ObservationListenerInventory
	r.Evidence.Observations[0].Payload = ObservationPayload{Kind: ObservationListenerInventory, Listener: &ListenerInventoryEntry{ListenerEntityID: "entity-listener", NamespaceEntityID: "entity-namespace", Protocol: TransportTCP, AddressFamily: AddressFamilyIPv4, BindSemantics: BindWildcard, Port: 80}}
	_, issues := ValidatePersistedEvaluatedRun(r)
	if !hasValidationCode(issues, CodeVisibilityInsufficientForAbsence) {
		t.Fatalf("non-target listener entry incorrectly validated absence: %v", issues)
	}
}

func TestListenerInventoryResultValidatesTypedScopeAndNamespaceVantage(t *testing.T) {
	tests := []struct {
		name   string
		mutate func(*EvidenceRun)
		code   ValidationCode
	}{
		{name: "reversed range", mutate: func(r *EvidenceRun) {
			r.Observations[0].Payload.ListenerInventoryResult.PortStart = 500
			r.Observations[0].Payload.ListenerInventoryResult.PortEnd = 400
		}, code: CodeInvalidValue},
		{name: "wrong namespace identity", mutate: func(r *EvidenceRun) {
			r.Entities = append(r.Entities, Entity{EntityID: "entity-other-namespace", Kind: EntityNetworkNamespace, DisplayLabel: "other", Identity: EntityIdentity{Kind: EntityNetworkNamespace, Namespace: &NamespaceIdentity{NamespaceInode: 8}}})
			r.Observations[0].Payload.ListenerInventoryResult.NamespaceEntityID = "entity-other-namespace"
		}, code: CodeVantageMismatch},
		{name: "incomplete limitation", mutate: func(r *EvidenceRun) {
			r.Observations[0].Limitations = []Limitation{{LimitationID: "limitation-000001", Code: LimitationPartialVisibility, Scope: LimitationScope{Kind: LimitationObservation}}}
		}, code: CodeInvalidValue},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			r := listenerAbsenceEvaluatedRun().Evidence
			tc.mutate(&r)
			if _, issues := ValidateEvidenceRun(r); !hasValidationCode(issues, tc.code) {
...
func listenerAbsenceEvaluatedRun() EvaluatedRun {
	...
	obs := Observation{ObservationID: "observation-000001", Kind: ObservationListenerInventoryResult, SubjectEntityIDs: []EntityID{"entity-namespace"}, VantageID: &vantageID, ObservedAt: t, Payload: ObservationPayload{Kind: ObservationListenerInventoryResult, ListenerInventoryResult: &ListenerInventoryResult{NamespaceEntityID: "entity-namespace", Protocol: TransportTCP, AddressFamily: AddressFamilyIPv4, BindSemantics: BindWildcard, PortStart: 1, PortEnd: 65535, MatchingListenerCount: 0}}, AcquisitionMethod: AcquisitionSyntheticFixture, SourceComponent: SourceSyntheticFixture, Sensitivity: SensitivitySanitizedDerived, Limitations: []Limitation{}}
